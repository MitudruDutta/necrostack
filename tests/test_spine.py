...
class TestOrganListensToValidation:
    ...
    @pytest.mark.parametrize("invalid_value", [
        123, 45.6, True, False, None, [1, 2], {"a": 1}
    ])
    def test_rejects_non_string_in_listens_to(self, invalid_value):
        ...
        OrganClass = type("TestOrgan", (Organ,), {
    ...
    @pytest.mark.parametrize("invalid_listens_to", [
        "not_a_list", 123, None, {"key": "value"}
    ])
    ...
    @pytest.mark.parametrize("valid_listens_to", [
        [], ["EVENT_A"], ["EVENT_A", "EVENT_B"], ["A", "B", "C", "D", "E"]
    ])
...
class TestHandlerReturnEnqueueing:
    ...
    async def test_single_event_return_enqueued(self):
        ...
        Emitter = type("Emitter", (Organ,), {"listens_to": ["START"], "handle": emitter_handle})
        Receiver = type("Receiver", (Organ,), {"listens_to": ["EMITTED"], "handle": receiver_handle})
        
        backend = StoppingBackend(spine_ref, max_events=10)
        spine = Spine(organs=[Emitter(), Receiver()], backend=backend, max_steps=10)
        spine_ref[0] = spine
    ...
    async def test_list_of_events_all_enqueued(self, num_events):
        ...
        spine = Spine(organs=[Emitter(), Receiver()], backend=backend, max_steps=20)
        spine_ref[0] = spine
    ...
    async def test_none_return_enqueues_nothing(self):
        ...
        NoneOrgan = type("NoneOrgan", (Organ,), {"listens_to": ["START"], "handle": none_handle})
        Receiver = type("Receiver", (Organ,), {"listens_to": ["NEVER"], "handle": receiver_handle})
        
        backend = StoppingBackend(spine_ref, max_events=10)
        spine = Spine(organs=[NoneOrgan(), Receiver()], backend=backend, max_steps=10)
        spine_ref[0] = spine
...
class TestOrganInvocationOrder:
    ...
    async def test_invocation_matches_registration_order(self, order):
        ...
        for name in order:
            ...
            OrganClass = type(f"Organ{name}", (Organ,), {
                "listens_to": ["TEST"],
                "handle": make_handle(name),
            })
            organs.append(OrganClass())
...
class TestMaxStepsEnforcement:
    ...
    async def test_raises_runtime_error_when_exceeded(self, max_steps):
        ...
        InfiniteOrgan = type("InfiniteOrgan", (Organ,), {
            "listens_to": ["LOOP", "START"],
            "handle": infinite_handle,
        })
        
        backend = StoppingBackend(spine_ref, max_events=1000)
        spine = Spine(organs=[InfiniteOrgan()], backend=backend, max_steps=max_steps)
        spine_ref[0] = spine
    ...
    async def test_no_error_when_under_limit(self):
        ...
        ChainOrgan = type("ChainOrgan", (Organ,), {
            "listens_to": ["START", "CHAIN"],
            "handle": chain_handle,
        })
        
        backend = StoppingBackend(spine_ref, max_events=50)
        spine = Spine(organs=[ChainOrgan()], backend=backend, max_steps=10)
        spine_ref[0] = spine
...
class TestSyncAsyncHandlerSupport:
    ...
    async def test_mixed_sync_async_handlers(self):
        ...
        SyncOrgan = type("SyncOrgan", (Organ,), {"listens_to": ["TEST"], "handle": sync_handle})
        AsyncOrgan = type("AsyncOrgan", (Organ,), {"listens_to": ["TEST"], "handle": async_handle})
        
        backend = StoppingBackend(spine_ref, max_events=10)
        spine = Spine(organs=[SyncOrgan(), AsyncOrgan()], backend=backend, max_steps=10)
        spine_ref[0] = spine
    ...
    async def test_async_handler_can_emit(self):
        ...
        AsyncEmitter = type("AsyncEmitter", (Organ,), {"listens_to": ["START"], "handle": async_emit})
        Receiver = type("Receiver", (Organ,), {"listens_to": ["ASYNC_OUT"], "handle": receiver})
        
        backend = StoppingBackend(spine_ref, max_events=10)
        spine = Spine(organs=[AsyncEmitter(), Receiver()], backend=backend, max_steps=10)
        spine_ref[0] = spine
    ...
    async def test_sync_handler_can_emit(self):
        ...
        SyncEmitter = type("SyncEmitter", (Organ,), {"listens_to": ["START"], "handle": sync_emit})
        Receiver = type("Receiver", (Organ,), {"listens_to": ["SYNC_OUT"], "handle": receiver})
        
        backend = StoppingBackend(spine_ref, max_events=10)
        spine = Spine(organs=[SyncEmitter(), Receiver()], backend=backend, max_steps=10)
        spine_ref[0] = spine
...
class TestSpineNoInternalQueue:
    ...
    def test_spine_has_no_queue_attributes(self):
        ...
        TestOrgan = type("TestOrgan", (Organ,), {"listens_to": ["TEST"], "handle": handle})
        
        backend = StoppingBackend(spine_ref)
        spine = Spine(organs=[TestOrgan()], backend=backend)
    ...
    async def test_spine_delegates_to_backend(self):
        ...
        TestOrgan = type("TestOrgan", (Organ,), {"listens_to": ["START"], "handle": handle})
        
        backend = TrackingBackend()
        spine = Spine(organs=[TestOrgan()], backend=backend, max_steps=100)
        spine_ref[0] = spine
