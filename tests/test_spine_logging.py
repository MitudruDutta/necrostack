...
async def test_structured_logging_contains_required_fields(log_capture):
    ...
    dispatch_records = [r for r in log_capture.records if "Dispatching" in r.getMessage()]
...
async def test_async_handler_does_not_crash_spine_logging(log_capture):
    ...
    spine = Spine(organs=[emitting_organ, stopping_organ], backend=backend, max_steps=100)
